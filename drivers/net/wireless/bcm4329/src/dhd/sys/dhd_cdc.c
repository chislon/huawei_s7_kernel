...
#define WIFI_MAC_FILE "/dev/wifimac"
...
static int
get_mac_address_from_file(struct ether_addr *addr)
{
	int ret = 0;
	struct file *fp;
	int nsize;
	...
	fp = filp_open(WIFI_MAC_FILE, O_RDONLY, 0);
	if (IS_ERR(fp)) {
		DHD_ERROR(("%s: cann't open mac file\n", __FUNCTION__));
		return -1;
	}

	nsize = kernel_read(fp, 0, mac_addr, sizeof(mac_addr));
	if (nsize != ETHER_ADDR_LEN) {
		DHD_ERROR(("%s: cann't read mac from file\n", __FUNCTION__));
		filp_close(fp, NULL);
		return -2;
	}
    ...
    if(i >= ETHER_ADDR_LEN)
    {
        memcpy(addr,ucMacDefault,ETHER_ADDR_LEN);
    }
#endif
	DHD_ERROR(("read mac address 0x%x:0x%x:0x%x:0x%x:0x%x:0x%x\n", 
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (get_mac_address_from_file(&ea)) {
		DHD_ERROR(("%s: read mac address from file failed, use onchip\n", __FUNCTION__));
	} 
    else 
    {
        memcpy(mac_addr, &ea, sizeof(ea));
        for(i=0;i<ETHER_ADDR_LEN;i++) {
            if(0 != mac_addr[i])  {
                break;
            }
        ...
        if(i < ETHER_ADDR_LEN) {
    		...
    		bcm_mkiovar("cur_etheraddr", (char *)&ea, sizeof(ea), iovbuf, sizeof(iovbuf));
	...
	pkt_filterp->id = htod32(100);

	/* Parse filter polarity. */
	pkt_filterp->negate_match = htod32(0);

	/* Parse filter type. */
	pkt_filterp->type = htod32(0);

	/* Parse pattern filter offset. */
	pkt_filterp->u.pattern.offset = htod32(0);
	...
	pkt_filterp->u.pattern.size_bytes = mask_size;
