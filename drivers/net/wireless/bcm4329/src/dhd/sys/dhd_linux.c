...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
    ...
    if (segment_eq(get_fs(), KERNEL_DS)) {
		memcpy(&driver, (char *)ifr->ifr_data + sizeof(wl_ioctl_t),
			sizeof(uint));
    } else {
...
int dhd_deep_sleep(struct net_device *dev, int flag)
{
 ...
 case 1: /* DEEPSLEEP ON*/
	...
	ioc.cmd = WLC_SET_VAR;
	ioc.buf = iovbuf;
	ioc.len = sizeof(iovbuf);
	ioc.set = TRUE;

	dhd_prot_ioctl(&dhd->pub, 0, &ioc, ioc.buf, ioc.len);
