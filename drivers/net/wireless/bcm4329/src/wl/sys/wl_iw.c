...
#ifdef ENABLE_DEEP_SLEEP
		dhd_deep_sleep(wl_ctl->dev, 1);

		dhd_customer_gpio_wlan_ctrl(WLAN_WAKE_OFF);
#else
#if defined(WL_IW_USE_ISCAN)
...
)
{
	...
	{
		dhd_customer_gpio_wlan_ctrl(WLAN_WAKE_ON);
		dhd_deep_sleep(dev, 0);
	}
#else
	if (g_onoff == G_WLAN_SET_OFF) {
...
)
{
	...
	if ((error = dev_wlc_ioctl(dev, WLC_SCAN_RESULTS, list, len))) {
		...
		if (g_scan_specified_ssid)
			kfree(list);
		//added by jiangyulei 
		if(BCME_NORESOURCE == error)
		{
			g_scan_specified_ssid = 0;
			return -EBUSY;
		}
		else if((BCME_NOTFOUND == error) || (BCME_NOTAP == error))
		{
			return 0;	
		}
		else
